...
function makeTime($delta)
	...
	public function getAccountCommentColor($accountID){
		include __DIR__ . "/connection.php";
		$query = $db->prepare("SELECT a.commentColor as 'commentColor', a.commentPrefix as 'commentPrefix', r.roleName as 'roleName' FROM accounts a JOIN roleassign ra ON (a.accountID = ra.accountID) JOIN roles r ON (r.roleID = ra.roleID) WHERE a.accountID = :accountID");
		$query->execute([':accountID' => $accountID]);
		$data = $query->fetchAll()[0];
		$commentColor = strtolower($data["roleName"]) != "player" ? $data["commentColor"] : "255,255,255";
		return $commentColor;
	}
	...
	public function getTimeBack($time, $userID = 0) {
	    $diff=time()-$time;
	    if($diff < 60) {
	        $count=$diff;
	        $pr="second";
	    ...
	    include __DIR__ . "/connection.php";
		$query = $db->prepare("SELECT a.commentPrefix as 'commentPrefix', r.roleName as 'roleName' FROM users u JOIN accounts a ON (u.extID = a.accountID) JOIN roleassign ra ON (a.accountID = ra.accountID) JOIN roles r ON (r.roleID = ra.roleID) WHERE u.userID = :id");
		$query->execute([':id' => $userID]);
		if ($query->rowCount() > 0) {
			$ud = $query->fetchAll()[0];
			$prefix = $ud["commentPrefix"];
			return(($prefix != "" and strtolower($ud["roleName"]) != "player") ? "skgd.xyz / $prefix / $out" : "skgd.xyz / $out");
		} else {
		    return($out);
		}
	}
