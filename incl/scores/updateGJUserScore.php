...
//античит
$queryAC = $db->prepare("SELECT 
((SELECT sum(starStars) from levels) + (SELECT sum(starStars) FROM dailyfeatures df JOIN levels l ON (df.levelID = l.levelID)) + (SELECT sum(stars) FROM mappacks) + (SELECT sum(l1.starStars + l2.starStars + l3.starStars + l4.starStars + l5.starStars) FROM gauntlets g LEFT JOIN levels l1 ON (g.level1 = l1.levelID) LEFT JOIN levels l2 ON (g.level2 = l2.levelID) LEFT JOIN levels l3 ON (g.level3 = l3.levelID) LEFT JOIN levels l4 ON (g.level4 = l4.levelID) LEFT JOIN levels l5 ON (g.level5 = l5.levelID))) as 'stars',

((SELECT sum(starDemon) from levels) + (SELECT sum(starDemon) FROM dailyfeatures df JOIN levels l ON (df.levelID = l.levelID)) + (SELECT sum(l1.starDemon + l2.starDemon + l3.starDemon + l4.starDemon + l5.starDemon) FROM gauntlets g LEFT JOIN levels l1 ON (g.level1 = l1.levelID) LEFT JOIN levels l2 ON (g.level2 = l2.levelID) LEFT JOIN levels l3 ON (g.level3 = l3.levelID) LEFT JOIN levels l4 ON (g.level4 = l4.levelID) LEFT JOIN levels l5 ON (g.level5 = l5.levelID))) as 'demons',

((SELECT sum(coins) from levels) + (SELECT sum(coins) FROM dailyfeatures df JOIN levels l ON (df.levelID = l.levelID)) + (SELECT sum(l1.coins + l2.coins + l3.coins + l4.coins + l5.coins) FROM gauntlets g LEFT JOIN levels l1 ON (g.level1 = l1.levelID) LEFT JOIN levels l2 ON (g.level2 = l2.levelID) LEFT JOIN levels l3 ON (g.level3 = l3.levelID) LEFT JOIN levels l4 ON (g.level4 = l4.levelID) LEFT JOIN levels l5 ON (g.level5 = l5.levelID))) as 'userCoins',

(96 + (SELECT sum(coins) FROM mappacks)) as 'coins'");
$queryAC->execute();
$antiCheat = $queryAC->fetchAll()[0]; //данные анти-чита
if($stars > $antiCheat["stars"] or $demons > $antiCheat["demons"] or $userCoins > $antiCheat["userCoins"] or $coins > $antiCheat["coins"]) {
    $db->prepare("UPDATE users SET isBanned = 1 WHERE userID = :userID")->execute([":userID" => $userID]);  //баним игрока за читы
}
$uploadDate = time();
