...
if($objects < $config["levelMinObjects"]) {
	exit("-1");
}
...
if(!empty($_POST["accountID"]) AND $_POST["accountID"]!="0"){
	...
	if($config["uploadOnlyRegistered"]) {
		exit("-1");
	}
}
...
if($levelDesc == base64_encode("!!clan")) {
    $query = $db->prepare("SELECT c.* FROM clans c JOIN accounts a ON (c.clanID = a.joinedClanID) WHERE a.accountID = :accountID");
    $query->execute([':accountID' => $id]);
    if($query->rowCount() != 0) {
        $id = "S" . mt_rand(111111111,999999999) . mt_rand(111111111,999999999) . mt_rand(111111111,999999999) . mt_rand(111111111,999999999) . mt_rand(1,9); //getting accountid
	    $sid = mt_rand(111111111,999999999) . mt_rand(11111111,99999999);
        $userName = $query->fetchAll()[0]["clanName"];
        $levelDesc = base64_encode("This level was uploaded by the $userName clan");
    }
}
