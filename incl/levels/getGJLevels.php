...
if($type==6 OR $type==17){ //featured
    $ratedAllShow = false;
	$params[] = "NOT starStars = 0";
	$order = "likes DESC,rateDate DESC,uploadDate";
}
...
if($onlyThisSeason) {
    $query = $db->prepare("SELECT * FROM seasons WHERE timeStart <= :time and timeEnd >= :time");
    $query->execute([":time" => time()]);
    $season = $query->fetchAll()[0];
    $timeStart = $season["timeStart"];
    $timeEnd = $season["timeEnd"];
    if($ratedAllShow) {
        $params[] = "(uploadDate >= $timeStart AND uploadDate <= $timeEnd) OR starStars != 0";
    } else {
        $params[] = "(uploadDate >= $timeStart AND uploadDate <= $timeEnd)";
    }
}
