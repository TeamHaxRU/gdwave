...
$fromBrowser = $ep->remove($_POST["fromBrowser"]);
//registering
...
if ($pass == 1) { //success
	...
	//result
	$token = sha1($userID."|".$id."|".rand(1, 999999)."|".microtime(true));
	$query7 = $db->prepare("INSERT INTO tokens (token, accountID) VALUES (:token, :accountID)");
	$query7->execute([':token' => $token, ':accountID' => $id]);
	if($fromBrowser == 1) {
	    echo(json_encode([
	        "ok" => 1, 
	        "token" => $token,
